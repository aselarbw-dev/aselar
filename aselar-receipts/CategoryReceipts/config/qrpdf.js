...
const generateReceiptWithQR = async (req, res) => {
  try {
    const { receiptData, htmlContent } = req.body; // Renamed from receiptsData for clarity; htmlContent optional/ignored if using buildReceiptPDF
    const pdfService = new PDFServiceJsPDF();

    // Chain: Generate PDF buffer → Upload to Firebase → Generate QR for the URL
    const pdfBuffer = await pdfService.generatePDFFromHTML(htmlContent, receiptData);
    const filename = pdfService.generateFilename(receiptData.receiptsNumber);
