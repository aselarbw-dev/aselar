...
class PDFServiceJsPDF {
  async generatePDFFromHTML(htmlContent, receiptsData = {}) {
    try {
      ...
      this.buildQuotationPDF(pdf, receiptsData);
  ...
  buildQuotationPDF(pdf, data) {
    ...
    pdf.text(`Receipt Number: ${data.receiptsNumber || 'N/A'}`, margin, yPos);
    yPos += 15;
  ...
  async uploadToFirebase(pdfBuffer, filename, metadata = {}) {
    try {
      const file = bucket.file(`quotations/${filename}`);
      const token = uuidv4();
      ...
      const publicUrl = `https://storage.googleapis.com/${bucket.name}/quotations/${filename}?token=${token}`;
